...
TASK = "Could you please create bar chart for the total sum of all sales by segment by year from the uploaded csv file and provide file to me?"


def sanitize_filename(filename):
    """Extract just the filename part, removing any path components"""
    # Use os.path.basename to get just the filename without path
    clean_filename = os.path.basename(filename)
    
    # If still empty or just path separators, use default
    if not clean_filename or clean_filename in ['/', '\\', '.', '..']:
        clean_filename = "generated_file.png"
...
async def main() -> None:
    async with (
        ...
        try:
            ...
            # 7. Download any generated files
            if thread:
                ...
                # Iterate through the async paged messages
                async for message in messages:
                    # print(f"Message: {message}")

                    # Check for file attachments in the message (includes images)
                    if hasattr(message, 'attachments') and message.attachments:
                        for attachment in message.attachments:
                            if hasattr(attachment, 'file_id'):
                                file_id = attachment.file_id
                                print(f"Found file attachment: {file_id}")
                                
                                # Get file details
                                file_info = await client.agents.files.get(file_id=file_id)
                                filename = file_info.filename if hasattr(file_info, 'filename') else "generated_file.png"
                                safe_filename = f"{file_id}_{sanitize_filename(filename)}"
                                
                                # Download file content
                                file_content_generator = await client.agents.files.get_content(file_id=file_id)
                                file_content_chunks = []
            ...
            # 8. Cleanup: Delete the thread, agent, and file
